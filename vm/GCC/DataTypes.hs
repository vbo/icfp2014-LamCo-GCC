...
getEnvFrame (EnvFrames tbl nxt) fid = if (Map.member fid tbl) then (tbl Map.! fid) else error ("accessing illegal frame: " ++ show fid)
...
initGCC :: GCC
initGCC = GCC 0 EmptyStack (stackPush EmptyStack ControlStop) (EnvFrames Map.empty 0) (-1)
